...
<html lang="en">
  <head>
    ...
    <title>WILDCARD — the classic card game</title>
  </head>
  <body>
    <div id="app"></div>
...
- Produces:
  ...
  - `Connection` interface and `createLoopbackPair(): [Connection, Connection]` (async message delivery via `queueMicrotask`; close notifies both ends exactly once, like real transports) for tests.
...
export interface LobbyInfo {
  ...
  config: RuleConfig;
  started: boolean;
  canStart: boolean; // host-computed: enough connected players to start (UI never recomputes this)
}
 ...
 * Close semantics mirror real transports: a self-initiated close fires the
 * local onClose too, and each end's onClose fires at most once even when
 * both ends close simultaneously.
 */
export function createLoopbackPair(): [Connection, Connection] {
  const make = (): LoopEnd => {
    ...
    const self: LoopEnd = {
      ...
      _closed() { open = false; notifyClose(); }
    };
    return self;
  };
  const a = make();
...
(Review addition: the suite must also cover later-seat removal keeping the turn
pointer, reverse-direction successor repair, `chooseSwapTarget` clearing, and a
pending wild4 from a removed player standing but being unchallengeable.)
...
except the guilty branch's draw, which must use the narrowed reference:
`drawFromDeck(s, accused.id, 4);` — `accusedId` alone stays `string | null`
...
export class HostSession {
  ...
  lobbyInfo(): LobbyInfo {
    ...
    return {
      ...
      // same presence condition startGame's guard uses — computed once, host-side
      canStart: players.filter((p) => p.connected).length >= 2
    };
  }
...
(Hardened during review — the shipped `src/net/peer.ts` differs from the block
above in three ways: `wrap()` fires `onClose` at most once with last-caller-wins
callback slots; peer-level errors after setup no longer destroy the peer, since
established data channels survive signaling loss and a broker hiccup must not
kill a live game; and a post-setup `disconnected` event triggers
`peer.reconnect()` so future joins/rejoins keep working.)
...
(Hardened during review — the shipped `session.svelte.ts` additionally wraps both
awaited peer calls in a 20s `withTimeout` (spec: no infinite spinners) and guards
them with a session epoch incremented by `leave()`: a cancelled or timed-out
connect can neither resurrect a left session on late success nor leak the late
loser's peer handle, which gets `destroy()`ed instead.)
...
<style>
  ...
  /* label is the ≥44px touch target; the checkbox itself may stay visually small */
...
<script lang="ts">
  ...
  // affordance comes precomputed from the host (LobbyInfo.canStart) — UI never recounts players
  const canStart = $derived(lobby?.canStart ?? false);
...
<style>
  ...
  .small { min-height: 44px; padding: 0 10px; font-size: 0.9em; } /* 44px touch-target floor */
...
(Unit tests gate the deploy. The e2e suite additionally runs in a separate
NON-GATING workflow — adjudicated during execution: the dev machine's VPN blocks
WebRTC entirely, so CI is the first environment that can actually execute the
e2e tests; a red e2e run must be treated as a real bug until proven environmental,
but it must not block Pages deploys.)

Also write `.github/workflows/e2e.yml`:

```yaml
name: E2E over real WebRTC
on:
  workflow_dispatch:
  push:
    branches: [main]
...
- [ ] **Step 3: Commit the workflow + README FIRST, then create the repo and push**

(Adjudicated ordering fix: the workflows must be committed before anything is
pushed, and the deploy workflow triggers on `main`, which won't exist if only
`build` is pushed — so seed `main` from `build` explicitly.)

```bash
git add .github README.md
git commit -m "ci: GitHub Pages deploy workflow and README"
gh repo create wildcard --public --source=. --push   # pushes current branch (build)
git push origin build:main                            # seed main → triggers deploy + e2e
...
(The workflow/README commit moved into Step 3; just ensure `build` is fully
pushed.)
...
3. `npm run e2e` — green in CI (the e2e.yml run; local execution is blocked by the dev machine's VPN).
